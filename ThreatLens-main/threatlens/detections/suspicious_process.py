...
SUSPICIOUS_PATTERNS = [
    (r"\b(?:nc|ncat|netcat)\b.*(?:-e|--exec|/bin/sh|cmd\.exe)", Severity.HIGH,
     "Netcat-style reverse shell",
     "Command and Control", "T1059 - Command and Scripting Interpreter"),
    (r"bash\s+-i\s*>&\s*/dev/tcp/\S+/\d+\s+0>&1", Severity.HIGH,
     "Bash reverse shell",
     "Command and Control", "T1059 - Command and Scripting Interpreter"),
    (r"powershell.*(?:tcpclient|networkstream|getstream|write\(\[text\.encoding\]::ascii\.getbytes|iex\s+\$)", Severity.HIGH,
     "PowerShell reverse shell behavior",
     "Command and Control", "T1059.001 - PowerShell"),
    (r"python\s+-c\s+.*(?:socket|subprocess|pty).*", Severity.HIGH,
     "Python-based reverse shell",
     "Command and Control", "T1059 - Command and Scripting Interpreter"),
    (r"perl\s+-e\s+.*(?:socket|exec|fork).*", Severity.HIGH,
     "Perl-based reverse shell",
     "Command and Control", "T1059 - Command and Scripting Interpreter"),
    (r"socat\s+.*exec:.*", Severity.HIGH,
     "Socat reverse shell",
     "Command and Control", "T1059 - Command and Scripting Interpreter"),
