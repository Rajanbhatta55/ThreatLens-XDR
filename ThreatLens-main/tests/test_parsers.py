...
class TestTimestampParsing:
    def test_iso_utc(self):
        ts = parse_timestamp("2026-01-15T08:30:01Z")
        assert ts.year == 2026
        assert ts.hour == 8

    def test_iso_fractional(self):
        ts = parse_timestamp("2026-01-15T08:30:01.123456Z")
        assert ts.microsecond == 123456

    def test_iso_no_z(self):
        ts = parse_timestamp("2026-01-15T08:30:01")
        assert ts.year == 2026

    def test_date_space_time(self):
        ts = parse_timestamp("2026-01-15 08:30:01")
        assert ts.hour == 8
...
class TestJsonParser:
    def test_parse_event_basic(self):
        raw = {
            "TimeCreated": "2026-01-15T08:30:01Z",
            "EventID": 4625,
            "Source": "Security",
            "Computer": "WS-PC01",
            "EventData": {
    ...
    def test_load_ndjson(self):
        with tempfile.NamedTemporaryFile(
            ...
            f.write('{"EventID": 4624, "TimeCreated": "2026-01-15T08:30:01Z", "Computer": "WS-01"}\n')
            f.write('{"EventID": 4625, "TimeCreated": "2026-01-15T08:30:05Z", "Computer": "WS-01"}\n')
            f.flush()
    ...
    def test_ndjson_with_malformed_line(self):
        with tempfile.NamedTemporaryFile(
            ...
            f.write('{"EventID": 4624, "TimeCreated": "2026-01-15T08:30:01Z"}\n')
            f.write('not valid json\n')
            f.write('{"EventID": 4625, "TimeCreated": "2026-01-15T08:30:05Z"}\n')
            f.flush()
            events = load_json_events(Path(f.name))
...
class TestSyslogParser:
    ...
    def test_rfc5424_iso_timestamp(self):
        ...
        dt = _parse_syslog_timestamp("2026-01-15T08:30:01.000Z")
        assert dt.year == 2026
        assert dt.hour == 8
    ...
    def test_parse_syslog_timestamp_iso_with_tz(self):
        ...
        dt = _parse_syslog_timestamp("2026-01-15T08:30:01+05:00")
        assert dt.year == 2026
        assert dt.hour == 8
    ...
    def test_parse_syslog_timestamp_iso_no_fractional(self):
        ...
        dt = _parse_syslog_timestamp("2026-01-15T08:30:01")
        assert dt.year == 2026

    def test_parse_syslog_timestamp_space_format(self):
        from threatlens.parsers.syslog_parser import _parse_syslog_timestamp

        dt = _parse_syslog_timestamp("2026-01-15 08:30:01")
        assert dt.year == 2026
    ...
    def test_rfc5424_full_parse(self):
        ...
        with tempfile.NamedTemporaryFile(
            ...
            f.write(
                "<165>1 2026-01-15T08:30:01Z server01 sshd 1234 ID47 "
    ...
    def test_syslog_rfc5424_maps_accepted_password_to_4624(self):
        ...
        line = (
            "<86>1 2026-01-15T08:30:05Z server01 sshd 4567 - "
