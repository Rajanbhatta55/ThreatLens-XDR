...
class TestYamlRules:
    def test_rule_matches(self):
        ...
        alerts = rule.analyze([make_failed_logon(datetime(2026, 1, 15, 8, 30, 0))])
        assert len(alerts) == 1
    ...
    def test_startswith_operator(self):
        ...
        events = [make_process_event(datetime(2026, 1, 15, 9, 0, 0), "powershell.exe", "test")]
    ...
    def test_not_equals_operator(self):
        ...
        events = [make_failed_logon(datetime(2026, 1, 15, 8, 30, 0))]
        assert len(rule.analyze(events)) == 1
    ...
    def test_not_contains_operator(self):
        ...
        events = [make_process_event(datetime(2026, 1, 15, 9, 0, 0), "cmd.exe", "dir C:\\")]
        assert len(rule.analyze(events)) == 1
    ...
    def test_gt_lt_operators(self):
        ...
        events = [make_failed_logon(datetime(2026, 1, 15, 8, 30, 0))]  # event_id=4625
        assert len(rule.analyze(events)) == 1
    ...
    def test_dot_notation_raw_field(self):
        ...
        event = LogEvent(
            timestamp=datetime(2026, 1, 15, 8, 30, 0),
            event_id=4625,
            source="Security",
